...
export default function Home() {
  ...
  const handleGenerate = async (provider: string, model: string, systemPrompt: string | null, maxTokens: number | undefined) => {
    if (!prompt.trim()) {
  ...
  const handleRegenerateWithNewPrompt = async (newPrompt: string) => {
    ...
    try {
      const response = await fetch('/api/generate-code', {
        ...
        body: JSON.stringify({
          prompt: newPrompt,
          model: "o4-mini", // Hardcode model for regeneration
          provider: "azure_openai", // Hardcode provider for regeneration
          maxTokens: undefined,
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <WelcomeView
        prompt={prompt}
        setPrompt={setPrompt}
          onGenerate={handleGenerate} // Pass handleGenerate to WelcomeView
        />
      )}
      <Toaster />
    </div>
